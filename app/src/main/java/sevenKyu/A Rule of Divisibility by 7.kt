...
fun seven(n:Long):LongArray {
    ...
    while (lastNumber >= 100) {
        val lastDigit = lastNumber % 10
        lastNumber = lastNumber / 10 - 2 * lastDigit
        steps++
    }
    return longArrayOf(lastNumber, steps.toLong())
}



fun sevenVer2(n: Long, i: Long = 0): LongArray =
    if (n > 99) sevenVer2(n / 10 - n % 10 * 2, i + 1)
    else longArrayOf(n, i)

fun main() {
    println(seven(477557101L).joinToString())
}